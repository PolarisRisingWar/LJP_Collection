...
def gen_law_relation(word2id_dict, law_label2index_path='law_label2index.pkl', doc_len=10, sent_len=100):
    law_file_order = pk.load(open(law_label2index_path, 'rb'))
    n_law = len(law_file_order)
    law_list = law_to_list('criminal_law.txt')
...
def get_law_graph_large(threshold, word2id_file, doc_len, sent_len):
    ...
    word2id_dict = pk.load(f)
    f.close()
    neigh_mat, law_index_matrix = gen_law_relation(word2id_dict, law_label2index_path='law_label2index_large.pkl', doc_len=doc_len, sent_len=sent_len)
    neigh_index = np.where(neigh_mat > threshold)
    # print(neigh_index)
    neigh_index = list(zip(*neigh_index))
